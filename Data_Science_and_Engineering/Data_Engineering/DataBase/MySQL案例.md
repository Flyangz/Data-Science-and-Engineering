...
属性：用户姓名、登录名、密码、证件类型、证件号码、手机号、邮箱、地址、积分、注册时间、生日、状态、级别、等级积分上限/下限、余额...

如果全部属性存储在一个表中，会出现如下问题：

插入异常：如只给某列填充一些数据，主键并不同时填充，这样会出错。

更新异常：如果将改变等级名称，则要扫全表，逐条改。

删除异常：如果删除某个等级的信息，会同时删除整行。

数据冗余（等级）、列过宽



范式判断：列出主属性、非主属性、候选码、函数关系，然后判断依赖关系

根据第三范式，上面整张表可以把“级别 + 等级积分上限/下限”单独成表

根据冷热分离，“登录名 + 密码 + 用户状态”单独成表

由于每个用户的地址可能有多个，“地址”单独成表
...
CREATE TABLE customer_login(
  ...
  login_name VARCHAR(20) NOT NULL UNIQUE KEY COMMENT '用户登录名',
...
CREATE TABLE customer_point_log(
  ...
  origin TINYINT UNSIGNED NOT NULL COMMENT '积分来源：0订单，1登陆，2活动',
...
CREATE TABLE customer_login_log(
  ...
  PRIMARY KEY pk_loginid(login_id) #分区时这一行要去掉，因为主键或唯一索引会是分区键的一部分
) ENGINE = innodb DEFAULT CHARST=utf8 COMMENT '用户登陆日志表';
...
# 改进，选选出id，然后join。假设customer_id是主键，且建立了联合索引
...
from(
    ...
    from product_comment
    where audit_status = 1 and product_id = 199726
    limit 0, 5) a join product_comment t on a.comment_id = t.comment_id;
```
...
- 逻辑备份结果为SQL语句，在恢复的时候执行备份的sql语句实现数据库数据的重现，适合于所有存储引擎。

- 物理备份是对数据库目录的拷贝，对内存表只备份结构（除非导出磁盘离线备份或者利用其他工具，如xtrabackup）
...
- -u 账户需具备的权限SELECT, RELOAD, LOCK TABLES, REPLICATION CLIENT, SHOW VIEW, PROCESS
- --single-transaction 开启事务来操作，只适用innodb。此期间不能有DDL语句执行。
- -l 依次锁表，一般myisam用。备份时锁住一个数据库下的表。如果混合引擎的表也用这个参数，和 `--single-transaction`互斥。只能保持一个db下的一致，这也是为什么通常规定所有表都用innodb。
- -x，锁所有db的所有表，备份期间所有表都不能写了。
- --master-data=[1/2] 在备份导出的文件里追加二进制binlog文件的位置和名称。时间恢复和新的slave实例用。1添加一个CHANGE MASTER语句 ，2则是把1放在注释里，配合 `--single-transaction`一起指定，否则就会lock all table
- -E 备份调度事件
...
还有指定时间恢复、实时备份binlog、xtrabackup备份和恢复、制定备份计划等暂略。
